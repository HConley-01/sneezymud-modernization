...
void zoneData::nukeMobs() {
  ...
  for (mob = character_list; mob; mob = mob2) {
    ...
    if ((mob->spec == SPEC_BOUNTY_HUNTER) ||
        dynamic_cast<TMonster*>(mob)->hatefield)
      continue;
    if (mob->isRideable())
      continue;
...
void TBeing::sendMobsGmcp() const {
  ...
  auto compare = [this](TMonster* a, TMonster* b) {
    return abs(a->GetMaxLevel() - this->GetMaxLevel()) <
           abs(b->GetMaxLevel() - this->GetMaxLevel());
  };
...
void TBeing::sendVitalsGmcp() const {
  ...
  if (hasClass(CLASS_MONK) || hasClass(CLASS_MAGE) ||
...
\"flags\": \"quiet\" }") %
      roomp->getZone()->zone_nr % roomp->getZone()->name.escapeJson();
    ...
    if (exitdata && (exitdata->to_room != Room::NOWHERE)) {
      ...
      if (isImmortal() ||
        ...
        if (exitdata->door_type != DOOR_NONE && !open) {
          exit_kw += format(", \"%s\": \"%s\"") % exDirs[door] %
                     sstring(exitdata->keyword).word(0);
        }
      }
    }
  }
  ...
  sstring msg =
    format(
      "room.info { \"num\": %d, \"name\": \"%s\", \"zone\": \"%d\", \"terrain\": \"%s\", \
\"details\": \"\", \"exits\": { %s }, \"exit_kw\": { %s }, \"coord\": { \"x\": %d, \"y\": %d, \"z\": %d, \"cont\": 0 } }") %
    roomp->number % roomp->name.escapeJson() % roomp->getZone()->zone_nr %
    sstring(TerrainInfo[roomp->getSectorType()]->name).escapeJson() %
    exits.substr(2) % exit_kw.substr(2) % roomp->getXCoord() %
    roomp->getYCoord() % roomp->getZCoord();
  desc->sendGmcp(msg, true);
}

void save_all() { descriptor_list->saveAll(); }

void sendToOutdoor(colorTypeT lev, const sstring& text,
  const sstring& text_tropic) {
  Descriptor* i;
  TBeing* ch;
...
void sendrpf(int tslevel, colorTypeT color, TRoom* rp, const char* msg, ...) {
  ...
  if (rp && msg) {
    ...
    vsnprintf(messageBuffer, cElements(messageBuffer), msg, ap);
    va_end(ap);
    for (StuffIter it = rp->stuff.begin(); it != rp->stuff.end() && (i = *it);
         ++it) {
      TBeing* tbt = dynamic_cast<TBeing*>(i);
      if (tbt && tbt->desc && !tbt->desc->connected && tbt->awake() &&
...
void colorAct(colorTypeT colorLevel, const sstring& str, bool hide,
  const TThing* t1, const TThing* obj, const TThing* t3, actToParmT type,
  const char* color, int tslevel) {
  bool colorize = 0;
  const TThing* to;
  int which = 0;
  char buf[256];
  ...
  if (!t3) {
    if (type == TO_VICT) {
      vlogf(LOG_MISC,
        format("There is no victim in coloract TOVICT %s is char.") %
          t1->getName());
      vlogf(LOG_MISC, format("%s") % str);
      return;
      ...
      type = TO_ROOM;
      vlogf(LOG_MISC,
        format("There is no victim in coloract TONOTVICT %s is char.") %
          t1->getName());
      vlogf(LOG_MISC, format("%s") % str);
    }
  }
  ...
  if (which) {
    ...
    for (StuffIter it = t1->roomp->stuff.begin(); it != t1->roomp->stuff.end();
         ++it) {
...
void act(const sstring& str, bool hide, const TThing* actor, const TThing* obj,
  const TThing* victim, actToParmT type, const char* color, int tslevel) {
  const char* strp;
  char* point;
  sstring i;
  ...
  if (!victim) {
    if (type == TO_VICT) {
      vlogf(LOG_BUG,
        format("Fatal missing victim in act() TO_VICT for '%s'.") % str);
      return;
    } else if (type == TO_NOTVICT) {
      type = TO_ROOM;
      vlogf(LOG_MISC,
        format("Missing victim in act() TO_NOTVICT for '%s', doing TO_ROOM.") %
          str);
    }
  }
  ...
  for (StuffIterConst it = list.begin(); it != list.end(); ++it) {
    ...
    if (to && to->desc && to->GetMaxLevel() > tslevel &&
        ...
        (to->canSee(actor) || !hide) && to->awake() &&
        (to->desc->connected < MAX_CON_STATUS) &&
        !(to->isPlayerAction(PLR_MAILING | PLR_BUGGING))) {
      x = 0;  // used to determine whether or not to capitalize the substitution
              // at start of line
        ...
        if (*strp == '$') {
          ...
          switch (*(++strp)) {
            ...
            case 'N':
              ...
              if ((type == TO_CHAR) && (actor == victim)) {
                ...
                if (!strncmp(strp + 1, "'s ", 3)) {
                  ...
                  char tmp_buffer[20];
                  snprintf(tmp_buffer, cElements(tmp_buffer), "%sself",
                    actor->hmhr());
                  i = tmp_buffer;
                }
              }
              ...
              break;
            case 'g':
              strncpy(ibuf, actor->roomp->describeGround().c_str(),
                cElements(ibuf) - 1);
              i = ibuf;
              break;
            case 'G':
              ...
              strncpy(ibuf, victim->roomp->describeGround().c_str(),
                cElements(ibuf) - 1);
              i = ibuf;
              break;
            case 'd':
              per =
                ((to == actor) ? FIRST_PERSON
                               : (!strlen(buf) ? THIRD_PERSON : SECOND_PERSON));
              strncpy(ibuf,
                actor
            ...
            case 'D':
              ...
              strncpy(ibuf,
                victim
                  ->yourDeity(your_deity_val,
                    ((to == victim)
                        ? FIRST_PERSON
                        : (strlen(buf) == 0 ? THIRD_PERSON : SECOND_PERSON)))
                  .c_str(),
                cElements(ibuf) - 1);
              i = ibuf;
              break;
            ...
            case 'o':
              ...
              strncpy(ibuf,
                dynamic_cast<const TBeing*>(obj) ? to->persfname(obj).c_str()
                                                 : to->objn(obj).c_str(),
                cElements(ibuf) - 1);
              i = ibuf;
              break;
            case 'O':
              ...
              strncpy(ibuf,
                dynamic_cast<const TBeing*>(victim)
                  ? to->persfname(victim).c_str()
                  : to->objn(victim).c_str(),
                cElements(ibuf) - 1);
              i = ibuf;
              break;
            case 'p':
      ...
      *(--point) = '\0';
      ...
      if (!color) {
        to->desc->output.push(
          CommPtr(new UncategorizedComm(format("%s\n\r") % s.cap())));
      } else {
        ...
        if (str.empty())
          ...
          to->desc->output.push(CommPtr(new UncategorizedComm(
            format("%s%s%s\n\r") % str % s.cap() % to->norm())));
        }
      }
    }
...
void Descriptor::updateScreenVt100(unsigned int update) {
  ...
  if (!ch->vt100()) {
    vlogf(LOG_MISC, format("%s in updateScreenVt100 and not vt (%d)") %
                      ch->getName() % update);
    return;
  }
  ...
  if (IS_SET(prompt_d.type, PROMPT_CLASSIC_ANSIBAR)) {
    ...
    if (update & CHANGED_HP) {
      snprintf(buf + strlen(buf), cElements(buf) - strlen(buf), VT_CURSPOS,
        ch->getScreen() - 2, 7);
      snprintf(buf + strlen(buf), cElements(buf) - strlen(buf), "%d",
        ch->getHit());
    }
    ...
    if (ch->hasClass(CLASS_DEIKHAN) || ch->hasClass(CLASS_CLERIC)) {
      if (update & CHANGED_PIETY) {
        snprintf(buf + strlen(buf), cElements(buf) - strlen(buf), VT_CURSPOS,
          ch->getScreen() - 2, 34);
        snprintf(buf + strlen(buf), cElements(buf) - strlen(buf), "%.1f",
          ch->getPiety());
      }
    ...
    if (update & CHANGED_HP) {
      snprintf(buf + strlen(buf), cElements(buf) - strlen(buf), VT_CURSPOS,
        ch->getScreen() - 2, 7);
      snprintf(buf + strlen(buf), cElements(buf) - strlen(buf), "%-5d",
        ch->getHit());
    }
    ...
    if (ch->hasClass(CLASS_DEIKHAN) || ch->hasClass(CLASS_CLERIC)) {
      ...
      if (update & CHANGED_LIFEFORCE) {
        snprintf(buf + strlen(buf), cElements(buf) - strlen(buf), VT_CURSPOS,
          ch->getScreen() - 2, 34);
        snprintf(buf + strlen(buf), cElements(buf) - strlen(buf), "%-4d",
          ch->getLifeforce());
      }
    } else {
      if (update & CHANGED_MANA) {
    ...
    if ((f = ch->fight()) != NULL) {
      if (f->sameRoom(*ch)) {
        ...
        snprintf(StTemp, cElements(StTemp), "<%s=%s>", fname(f->name).c_str(),
          prompt_mesg[ratio]);
    ...
    if (ch->isImmortal()) {
      if (update & CHANGED_ROOM) {
        snprintf(buf + strlen(buf), cElements(buf) - strlen(buf), VT_CURSPOS,
          ch->getScreen() - 1, 62);
        snprintf(buf + strlen(buf), cElements(buf) - strlen(buf), "%-6d",
          ch->roomp->number);
      }
    } else {
#if FACTIONS_IN_USE
    ...
    if (IS_SET(update, CHANGED_MUD)) {
      snprintf(buf + strlen(buf), cElements(buf) - strlen(buf), VT_CURSPOS,
        ch->getScreen(), 1);
      snprintf(buf + strlen(buf), cElements(buf) - strlen(buf), "   %8s   ",
        GameTime::hmtAsString(GameTime::hourminTime()).c_str());
    }
    ...
    if (update & CHANGED_EXP) {
      ...
      for (iClass = MAGE_LEVEL_IND; iClass < MAX_CLASSES; iClass++) {
        if (ch->getLevel(iClass)) {
          double iNeed =
            getExpClassLevel(ch->getLevel(iClass) + 1) - ch->getExp();
    ...
    if (update & CHANGED_GOLD) {
      snprintf(buf + strlen(buf), cElements(buf) - strlen(buf), VT_CURSPOS,
        ch->getScreen(), 62);
      snprintf(buf + strlen(buf), cElements(buf) - strlen(buf), "%-8d",
        ch->getMoney());
    }
  }
...
void Descriptor::updateScreenAnsi(unsigned int update) {
  ...
  if (!ch->ansi()) {
    vlogf(LOG_MISC, format("%s in updateScreenAnsi and not ansi (%d)") %
                      ch->getName() % update);
    return;
  }
  ...
  if (ch->hasClass(CLASS_DEIKHAN) || ch->hasClass(CLASS_CLERIC))
    ...
    current_mana =
      (int)(10 * ((double)ch->getMana() / (double)ch->manaLimit()));
  ...
  if (IS_SET(prompt_d.type, PROMPT_CLASSIC_ANSIBAR)) {
    ...
    if (update & CHANGED_HP) {
      ...
      buf += format("%s%-5d ") % (current_hit > 2 ? VT_BOLDTEX : ANSI_RED) %
             ch->getHit();
      buf += ANSI_BLUE;
    ...
    if (IS_SET(update, CHANGED_MANA) || IS_SET(update, CHANGED_PIETY) ||
        IS_SET(update, CHANGED_LIFEFORCE)) {
      buf += format(VT_CURSPOS) % (ch->getScreen() - 2) % 34;

      if (ch->hasClass(CLASS_DEIKHAN) || ch->hasClass(CLASS_CLERIC))
        buf += format("%s%-5.1f ") % (current_mana ? VT_BOLDTEX : ANSI_RED) %
               ch->getPiety();
      else if (ch->hasClass(CLASS_SHAMAN))
        buf += format("%s%-5d ") % (current_mana ? VT_BOLDTEX : ANSI_RED) %
               ch->getLifeforce();
      else
        buf += format("%s%-5d ") % (current_mana ? VT_BOLDTEX : ANSI_RED) %
               ch->getMana();
    ...
    if (IS_SET(update, CHANGED_MOVE)) {
      ...
      buf += format("%s%-5d ") % (current_moves ? VT_BOLDTEX : ANSI_RED) %
             ch->getMove();
      buf += ANSI_BLUE;
    ...
    if (IS_SET(update, CHANGED_MUD)) {
      ...
      buf +=
        format("   %8s   ") % GameTime::hmtAsString(GameTime::hourminTime());
    }
      ...
      for (iClass = MAGE_LEVEL_IND; iClass < MAX_CLASSES; iClass++) {
        if (ch->getLevel(iClass)) {
          ...
          if (ch->getLevel(iClass) >= MAX_MORT)
            ...
            snprintf(StTemp, cElements(StTemp),
              ((ch->getExp() < 100) ? "%.3f" : "%.0f"), iNeed);
...
// base class
sstring Comm::getComm() { return getText(); }

// UncategorizedComm
UncategorizedComm::UncategorizedComm(const sstring& t) { text = t; }

sstring UncategorizedComm::getText() { return text; }

GmcpComm::GmcpComm(const sstring& t) { text = t; }

sstring GmcpComm::getText() { return text; }
...
sstring CmdMsgComm::getText() { return text; }
...
PromptComm::PromptComm(time_t t, int h, int m, float p, int l, int mv, int g,
  int r, const sstring& txt) {
  time = t;
  hp = h;
  mana = h;
  piety = p;
...
// SoundComm
SoundComm::SoundComm(const sstring& st, const sstring& u, const sstring& s,
  const sstring& t, int v, int p, int r, int c) {
  soundtype = st;
  url = u;
  text = s;
  type = t;
